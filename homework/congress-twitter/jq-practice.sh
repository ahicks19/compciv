...
echo '6.'
cat data-hold/legislators-current.json | jq '.[] .bio .religion' | sort | uniq -c | sort -r
...
echo '13.'
cat data-hold/legislators-social-media.json | jq '.[] .social .facebook' | grep -v 'null' | wc -
...
cat data-hold/legislators-social-media.json | \
>   jq --raw-output '.[]  | [.id .bioguide, .social .twitter] | @csv' \
>   | grep -v '[.social .twitter]' | head -n 10
...
echo '17.'
cat data-hold/congress-twitter-profiles.json | \
>   jq '.[] .followers_count' | sort -nr | head -n 1
...
echo '19.'
cat data-hold/congress-twitter-profiles.json | \
> jq --raw-output '.[] | [.screen_name, .statuses_count, .followers_count, .status .creat_at] | @csv' | head -n 10
